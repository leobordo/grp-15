...
public function getPromozionePublic($chiave){
    ...
    if ($pr==null) {
        $message='Promo non esistente. Verrai reindirizzato alla home tra 5 secondi...';
            return view('errors.Error404')->with('err',$message);
    }
    
    if ($pr->Scadenza >= date('Y-m-d') || (auth()->user() != null && auth()->user()->Livello==2) ) {
...
public function showRisultatiPromo(Request $request)
{
    if ($request->ajax()) {
        ...
        return view('partials.risultati_ricerca_promozioni', ['results' => $results]);
        }
